...
- [三、核心业务对象](#3-核心业务对象)
  ...
  - [（二）业务状态流转](#32-业务状态流转)
- [四、功能模块设计](#四功能模块设计)
  - [（一）模块一：[模块名称]](#41-模块一模块名称)
  - [（二）模块二：[模块名称]](#42-模块二模块名称)
  - [（3）模块三：[模块名称]](#43-模块三模块名称)
  - [（4）模块四：[模块名称]](#44-模块四模块名称)
  - [（5）数据交换需求](#45-数据交换需求)
    - [① 与其他系统的数据交换](#（1）与其他系统的数据交换)
    - [② 数据同步规则](#（2）数据同步规则)
- [五、业务约束与体验要求](#五业务约束与体验要求)
  - [（一）用户体验要求](#（1）用户体验要求)
  - [（二）业务规则约束](#（2）业务规则约束)
  - [（三）安全与合规](#（3）安全与合规)
- [六、附录](#六附录)
...
> **本详细设计文档必须遵从以下排版参数，以确保打印与归档的统一性。**
...
本文档模板使用 Markdown 格式编写以支持版本控制，生成正式 Word 文档时需按上述格式调整：
...
2. 设置正文默认字体为宋体小四，行距1.5倍
3. 设置段落首行缩进2字符
4. 使用"多级列表"自动编号，按标题级别设置序号格式
